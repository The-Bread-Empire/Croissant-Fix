...
async def on_message(message):
    ...
    if status == 1:
        ...
        await client.change_presence(
            activity=discord.Activity(
                ...
                name="you eat cheese at 3am in the morning"))
    elif status == 4:
        await client.change_presence(
            activity=discord.Game(name="roasting you"))
    elif status == 5:
    ...
    if message.author.id == 515704039686668310:
        ...
        if randannoy == 1:
            await message.channel.send(
                f"{message.author.name} thinks im a good bot :smirk:")
        elif randannoy == 2:
            await message.channel.send("I am a good bot :smirk:")

    answers = {"rock": "skizzors", "skizzors": "paper", "paper": "rock"}
    ...
    if message.content == "join a server":
        await message.channel.send(
            "Join code for croissant bot: https://discord.com/api/oauth2/authorize?client_id=764150242438283335&permissions=8&scope=bot"
        )
    if (1 == 1):
        ...
        if gunrand == 1:
            await message.channel.send(
                f"I told you guns are dangerous. You shot yourself in the {bodypart}"
            )
